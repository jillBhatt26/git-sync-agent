...
1. **Safety & Intent Detection**:
   ...
   - If neither are provided: target all git repos in the base directory to pull their default branch into the current working branch.

2. **Repo Discovery & Sync Execution**:
   - Use your `run_command` tool to execute the sync script based on the safety rules determined in step 1.
   - For the targeted repo(s), construct a script that:
     ...
     4. Determines the target branch (e.g., via `git symbolic-ref -q refs/remotes/origin/HEAD`).
...
5. **Edges**: 
   - No auto-resolve of conflicts. If a pull fails or a conflict is detected during the automated execution, stop and report it to the user so they can handle it manually.
